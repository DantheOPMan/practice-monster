...
namespace PracticeMonster
{
    public class Blaze : Ability
    {
        ...
        public override float Activate(Battle battle, Monster owner, Move move, Monster target = null)
        {
            if (owner.CurrentHP <= owner.Data.MaxHP / 3 && move.Type == "Fire")
            {
                BattleUIManager.Instance.Log($"{owner.Nickname}'s Blaze activated, increasing the power of {move.Name}!");
                return 1.5f;
            }
            return 1.0f;
        }
    ...
    public class Intimidate : Ability
    {
        ...
        public override float Activate(Battle battle, Monster owner, Move move, Monster target = null)
        {
            if (target != null)
            {
                ...
                BattleUIManager.Instance.Log($"{owner.Nickname}'s Intimidate activated, lowering {target.Nickname}'s Attack!");
            }
            return 1.0f;
        }
    }
    ...
    public class Overgrow : Ability
    {
        ...
        public override float Activate(Battle battle, Monster owner, Move move, Monster target = null)
        {
            ...
            {
                BattleUIManager.Instance.Log($"{owner.Nickname}'s Overgrow activated, increasing the power of {move.Name}!");
                return 1.5f;
            }
            return 1.0f;
        }
    ...
    public class SwiftSwim : Ability
    {
        ...
        public override float Activate(Battle battle, Monster owner, Move move, Monster target = null)
        {
            if (battle.Weather == Weather.Rain)
            {
                ...
                BattleUIManager.Instance.Log($"{owner.Nickname}'s Swift Swim activated, doubling its Speed in rain!");
            }
            return 1.0f;
        }
    }
    ...
    public class Sturdy : Ability
    {
        ...
        public override float Activate(Battle battle, Monster owner, Move move, Monster target = null)
        {
            if (owner.CurrentHP == owner.Data.MaxHP && owner.CurrentHP > 0 && owner.CurrentHP - target.CalculateDamage(owner) <= 0)
            {
                owner.CurrentHP = 1;
                BattleUIManager.Instance.Log($"{owner.Nickname}'s Sturdy activated, preventing it from being knocked out!");
            }
            return 1.0f;
        }
    }*/
